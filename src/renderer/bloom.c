...
EngineResult bloom_init(VulkanContext *vk) {
    ...
    if ((res = vk_create_bloom_scene_skinned_3d_pipeline(vk)) != ENGINE_SUCCESS) return res;
...
void bloom_shutdown(VulkanContext *vk) {
    ...
    if (b->scene_skinned_pipeline)    vkDestroyPipeline(vk->device, b->scene_skinned_pipeline, NULL);
